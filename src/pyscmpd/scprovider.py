		...
		try:
			...
			while True:
				...
				if len(groups) == 0:
					break
			'''
			groups = ResourceProvider.sc.get("/groups", limit=ResourceProvider.reqLimit)
		...
		try:
			...
			while True:
				...
				if len(allUsers) == 0:
					break
			'''
			allUsers = ResourceProvider.sc.get("/users", limit=ResourceProvider.reqLimit)
			...
			while True:
				...
				for track in tracks:
					...
					try:
						...
						logging.warn("Unable to add track (no stream URL): %s" % track.title)
